...
double calculatePitch(double accelX, double accelY, double accelZ);
double calculateRoll(double accelY, double accelZ);
...
void imuGetData(imudata_t *accel, imudata_t *gyro, imudata_t *mag, eulerangle_t *angle, float *compass)
{
  ...
  //eulerangle_t tempAngle = calculateEulerAngles();
  angle->pitch = calculatePitch(imu.calcAccel(imu.ax), imu.calcAccel(imu.ay), imu.calcAccel(imu.az));
  angle->roll = calculateRoll(imu.calcAccel(imu.ay),imu.calcAccel(imu.az));
  //angle->yaw = tempAngle.yaw;
...
//Calculation addapted from https://engineering.stackexchange.com/questions/3348/calculating-pitch-yaw-and-roll-from-mag-acc-and-gyro-data
double calculatePitch(double accelX, double accelY, double accelZ)
{
